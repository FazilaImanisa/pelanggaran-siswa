...
exports.filterPelanggaran = async (request, response) => {
    /** filter tanggal awal sm akhir */
    let start = request.body.start // awal
    let end = request.body.end // akhir
...
exports.addData = async(request, response) => {
    ...
    // Proses pengurangan poin dari siswa yg melanggar
        // 1. mengambil poin dari siswa yang bersangkutan (dari tabel poin siswa)
        let siswa = await siswaModel.findOne({
            where: {id_siswa: request.body.id_siswa}
        })
        let poinSiswa = siswa.poin

        // 2. mengambil nilai poin dari tiap pelanggarannya (dari detail pelanggaran)
